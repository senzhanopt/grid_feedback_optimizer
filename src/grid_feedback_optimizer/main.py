...
def main(file_path: str, save_path: str | None = None, max_iter: int = 1000, tol: float = 1e-3,
          delta_p: float = 1.0, delta_q: float = 1.0, algorithm: str = "gp", 
          alpha: float = 0.5, alpha_v: float = 10.0, 
          alpha_l: float = 10.0, alpha_t: float = 10.0, no_record_iterates: bool = False,
    ...
    results = solve(network, max_iter = max_iter, tol = tol, delta_p = delta_p, delta_q = delta_q,
                    ...
                    alpha_t = alpha_t, record_iterates = not no_record_iterates, solver = solver,
                    loading_meas_side = loading_meas_side, rel_tol = rel_tol, rel_tol_line = rel_tol_line, **solver_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("file_path", type=str, help="Path to network JSON or Excel file")
